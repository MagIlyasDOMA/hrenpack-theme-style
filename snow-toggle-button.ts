...
document.addEventListener('DOMContentLoaded', () => {
    if (snow_button) {
        if (snowSvg) snowSvg.src = snowEnabled ? snowToggleButtonImages.on : snowToggleButtonImages.off
        snow_button.addEventListener('click', () => {
            if (snowSvg) snowSvg.src = snowEnabled ? snowToggleButtonImages.onAnimation : snowToggleButtonImages.offAnimation
        })
    }
})
